...
export const analyzeTasks = async (tasks: StaffTask[]): Promise<ManagementSummary> => {
  // Use strictly compliant initialization with process.env.API_KEY
  ...
  const prompt = `
    ...
    6. Highlight:
       ...
       - Pending Approval tasks (status is 'Pending Approval') - these are tasks waiting for lead sign-off.
       - Overdue tasks (dueDate is before today and status is not 'Completed').
