...
import { StaffTask, TaskStatus, User, Frequency, Brand, ServiceType, TaskComment } from '../types';
                  ...
                  >
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${task.staffName}`} className="w-9 h-9 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm" />
                        <div>
                          <p className="text-xs font-black text-slate-800 dark:text-white leading-none">{task.staffName}</p>
                          <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                            {users.find(u => u.name === task.staffName)?.role || 'Member'}
                          </p>
                        </div>
                      </div>
                    </td>
                                    ...
                                    <div className="flex justify-between items-center mb-3">
                                      <div className="flex items-center gap-2">
                                        <div className="w-6 h-6 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-[8px] font-black text-slate-500 overflow-hidden">
                                          <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${c.authorName}`} alt="" />
                                        </div>
                                        <div>
